...
**Not installed yet?** Follow [`SELF_HOST.md`](../admin-guide/self-host.mdx) (pick §1 SQLite, §2 Postgres+pgvector, or §3 production), then use the [verification checklist](../admin-guide/self-host.mdx#after-install-verification-checklist).
...
| **Can't connect to database** | Check Postgres running + `DATABASE_URL`. Dev: [`SELF_HOST.md` §2](../admin-guide/self-host.mdx#2-local-development-with-postgresql--pgvector) — `docker compose -f deploy/docker-compose.postgres.yml up -d` |
| **Install verify fails** (`backend` not `postgresql`) | Set `DATABASE_URL` + `BRIEFR_REQUIRE_POSTGRES=1`, restart backend — see [SELF_HOST verify checklist](../admin-guide/self-host.mdx#after-install-verification-checklist) |
| **Backup / restore failed** | Need age key for encrypted archives. `briefr-restore.sh --list` · see [SELF_HOST.md](../admin-guide/self-host.mdx#updates--backups) |
...
| **Can't log in** | Complete first-run setup once ([SELF_HOST](../admin-guide/self-host.mdx)). Fix `ALLOWED_ORIGINS` for CORS |
...
| **Installed Postgres but still on `npm run dev`** | `npm run dev` is local development only (`:5173`). Production uses [SELF_HOST §3](../admin-guide/self-host.mdx#3-production-debian--systemd--nginx): `briefr-install.sh`, nginx on `:80`, `BRIEFR_ENV=production` |
...
| **`database is locked` (SQLite dev)** | SQLite allows one writer — stop parallel pytest/dev servers or switch to Postgres ([SELF_HOST §2](../admin-guide/self-host.mdx#2-local-development-with-postgresql--pgvector)) |
...
- [`SELF_HOST.md`](../admin-guide/self-host.mdx) — install paths and verification checklist
- [PRODUCT_STATUS.md](https://github.com/Soldier0x0/briefr/blob/main/docs/PRODUCT_STATUS.md) — what's supposed to work today
- [HOW_IT_WORKS.md](./how-it-works.mdx) — architecture if you need context
