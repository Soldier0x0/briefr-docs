...
Do not paste Archify HTML into Markdown. Use `<ArchifyDiagram id="..." title="..." />` on MDX pages. Migrated CommonMark (`.md`) uses a same-origin iframe in an `.archify-embed` shell sized from the SVG `viewBox` (catalog `viewBox: [w, h]`). Iframe `src` is `/diagrams/<id>?theme=dark&present=1&embed=1` (no `.html` — Docusaurus 301s `*.html?query` and drops the query). Never use a shared pixel height.
